...
const ensureDbFiles = async () => {// ✦ Sistema de Roles LATAM ✦ Swill
...
import { ROLES } from './roles-config.js' // ✅ Nuevo sistema de roles
...
// ✅ Guardar roles.json
// ------------------------------------------------------------
const saveRoles = async (rolesObj) => {
  try {
    await ensureDbFiles()
...
// ------------------------------------------------------------
// ✅ API pública (tu sistema original)
// ------------------------------------------------------------
const listRole = (type) => {
...
const addRole = async (type, jid, meta = '', opts = {}) => {
  await loadRoles()
...
const removeRole = async (type, jid, opts = {}) => {
  await loadRoles()
  const roles = global.roles
  const num = toNum(jid)
...
// ------------------------------------------------------------
// ✅ NUEVO SISTEMA: integración con roles-config.js
// ------------------------------------------------------------

/**
 * Devuelve el rol REAL del usuario según roles.json
 */
export function getUserRole(jid) {
  if (!jid) return 'user'

  if (hasRole('roowner', jid)) return 'roowner'
  if (hasRole('owners', jid)) return 'owner'
  if (hasRole('mods', jid)) return 'mod'
  if (hasRole('prems', jid)) return 'premium'
  if (hasRole('suittag', jid)) return 'suittag'

  return 'user'
}

/**
 * Devuelve la información completa del rol desde roles-config.js
 */
export function getRoleInfo(jid) {
  const roleId = getUserRole(jid)
 ...
 */
export function roleHasPermission(jid, permId) {
  const role = getRoleInfo(jid)
...
export {
  toNum, jidFull,
  loadRoles, saveRoles, appendLog,
  listRole, hasRole, addRole, removeRole
}
  try {
    await fs.mkdir(DB_DIR, { recursive: true })
    try { await fs.access(ROLES_PATH) } catch {
    ...
    try { await fs.access(LOG_PATH) } catch { await fs.writeFile(LOG_PATH, '[]') }
  } catch (e) { console.error('roles.ensureDbFiles error', e) }
}

const toNum = jid => String(jid || '').replace(/@s\.whatsapp\.net/g, '').replace(/\D/g, '')
const jidFull = num => (num && num.includes('@')) ? num : `${toNum(num)}@s.whatsapp.net`

// Validación básica de número/JID
const isValidNum = n => {
...
const backupFile = async (filePath) => {
  try {
    ...
    // no bloquear si falla el backup
  }
}

// Mutex simple para escrituras
let _rolesLock = false
const withLock = async (fn) => {
  while (_rolesLock) await new Promise(r => setTimeout(r, 50))
  _rolesLock = true
...
// Carga cache: roles.json + mods desde readMods()
const loadRoles = async () => {
  await ensureDbFiles()
  try {
    const raw = await fs.readFile(ROLES_PATH, 'utf8')
    ...
    const roles = {
      roowner: Array.isArray(parsed.roowner) ? parsed.roowner.map(toNum) : [],
      owners: Array.isArray(parsed.owners) ? parsed.owners.map(o => Array.isArray(o) ? [toNum(o[0]), o[1] || '', !!o[2]] : [toNum(o), '', false]) : [],
      mods: Array.isArray(modsRaw) ? modsRaw.map(x => jidFullMods(x)) : [],
      suittag: Array.isArray(parsed.suittag) ? parsed.suittag.map(x => jidFull(x)) : [],
      prems: Array.isArray(parsed.prems) ? parsed.prems.map(x => jidFull(x)) : []
    }
...
const saveRoles = async (rolesObj) => {
  try {
    await ensureDbFiles()
    // Guardar mods usando saveMods para mantener compatibilidad
    if (rolesObj.mods) {
      const modsToSave = rolesObj.mods.map(x => toNum(x) + '@s.whatsapp.net')
      await saveMods(modsToSave)
    }
    const toSave = {
      roowner: Array.isArray(rolesObj.roowner) ? Array.from(new Set(rolesObj.roowner.map(toNum))) : [],
      owners: Array.isArray(rolesObj.owners) ? rolesObj.owners.map(o => Array.isArray(o) ? [toNum(o[0]), o[1] || '', !!o[2]] : [toNum(o), '', false]) : [],
      suittag: Array.isArray(rolesObj.suittag) ? Array.from(new Set(rolesObj.suittag.map(n => toNum(n) + '@s.whatsapp.net'))) : [],
      prems: Array.isArray(rolesObj.prems) ? Array.from(new Set(rolesObj.prems.map(n => toNum(n) + '@s.whatsapp.net'))) : []
    }

    // Backup y escritura atómica con lock
    await withLock(async () => {
      await backupFile(ROLES_PATH)
      await fs.writeFile(ROLES_PATH, JSON.stringify(toSave, null, 2))
    })
...
const appendLog = async (entry) => {
  try {
    ...
    // también registra en el log de mods si aplica
    if (normalized.type === 'mods' || (normalized.action && normalized.action.includes('mod'))) {
      appendModLog(normalized)
    }
  } catch (e) { console.error('roles.appendLog error', e) }
}
...
const removeRole = async (type, jid, opts = {}) => {
  ...
  if (type === 'owners') {
    ...
    if (!roles.roowner.includes(num)) return false
    // Protección: no permitir eliminar el último roowner
    if (roles.roowner.length <= 1 && roles.roowner.includes(num)) {
      throw new Error('CANNOT_REMOVE_LAST_ROOWNER')
    }
    roles.roowner = roles.roowner.filter(x => x !== num)
  } else {
    roles[type] = (roles[type] || []).filter(x => toNum(x) !== num)
