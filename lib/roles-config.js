...
let handler = async (m, { conn, args, command }) => {
  ...
  if (command === 'mutelog') {
    if (mutelog.length === 0)
      return conn.reply(m.chat, '📄 *El registro de muteos está vacío.*', m)

    let text = '📄 *Registro de muteos:*\n\n'
    for (let entry of mutelog.slice(-20).reverse()) {
      text += `• @${entry.target?.split('@')[0]} — ${entry.action} por @${entry.by?.split('@')[0]}\n`
    }

    return conn.reply(m.chat, text, m, {
      mentions: mutelog.map(e => e.target).filter(Boolean)
    })
  }
  ...
  if (command === 'clearmutelog') {
    mutelog = []
    saveLog()
    return conn.reply(m.chat, '🧹 *Registro de muteos limpiado.*', m)
  }
}
