...
/**
 * Carga segura de @whiskeysockets/baileys
 * - Intenta import dinámico (ESM)
 * - Si falla, intenta require usando module.createRequire
 * - Si todo falla, deja un objeto vacío para evitar crashes y loggea el error
 */
let _bx = {}
try {
  _bx = await import('@whiskeysockets/baileys')
} catch (e) {
  try {
    const { createRequire } = await import('module')
    const require = createRequire(import.meta.url)
    _bx = require('@whiskeysockets/baileys')
  } catch (e2) {
    console.error('store.js: no se pudo cargar @whiskeysockets/baileys', e2)
    _bx = {}
  }
}
...
class Semaphore {
  constructor(max) { this.max = max; this.used = 0; this.q = [] }
  async acquire() {
    if (this.used < this.max) { this.used++; return }
    await new Promise(res => this.q.push(res)); this.used++
  }
  release() { this.used--; const n = this.q.shift(); if (n) n() }
}

class FastQueue {
  constructor({ concurrency = 64, timeoutMs = 12000 } = {}) {
  ...
  async run(fn) {
    ...
    try {
      const to = new Promise((_, rej) => setTimeout(() => rej(new Error('PluginTimeoutError')), this.timeoutMs))
...
function _onlyDigits(s = '') { return (String(s).match(DIGITS) || []).join('') }

function _baseJid(s) {
  let x = String(s || '').trim()
  if (!x) return ''
  if (x === 'status@broadcast' || x === 'broadcast@status') return 'status@broadcast'
  x = x.replace(LID, '@s.whatsapp.net')
  if (x.includes('@g.us')) return x.endsWith('@g.us') ? x : `${x.split('@')[0]}@g.us`
  x = x.replace(DEVICE, '')
  if (x.includes('@s.whatsapp.net')) return `${x.split('@')[0]}@s.whatsapp.net`
  if (x.includes('@')) return x
  return ''
}

function _asUserJid(s) {
  const d = _onlyDigits(String(s || '').replace(NONSTD, ''))
  if (d.length >= 8 && d.length <= 20) return `${d}@s.whatsapp.net`
  return null
}

function _asGroupJid(s) {
  const d = _onlyDigits(String(s || '').replace(SNUM, ''))
  if (d.length >= 8 && d.length <= 30) return `${d}@g.us`
  return null
}
    ...
    try {
      ...
      for (const contact of contacts) {
        ...
        row = {
          ...
          ...contact,
          id,
          ...(isGroup ? { subject: contact.subject || contact.name || row.subject || '' } : { name: contact.notify || contact.name || row.name || row.notify || '' })
        }
        conn.chats[id] = row
      ...
      keys: {
        get: (type, ids) => {
          ...
          return ids.reduce((d, id) => {
            let v = keys[key]?.[id]
            if (v && type === 'app-state-sync-key') v = proto.AppStateSyncKeyData?.fromObject?.(v) || v
            if (v) d[id] = v
            return d
          }, {})
        },
        set: (data) => {
          for (const _k in data) {
            const key = KEY_MAP[_k]
            keys[key] = keys[key] || {}
            Object.assign(keys[key], data[_k])
          }
          saveState(false)
        }
      }
...
async function tryRehydrateSession(conn, jid) {
  try {
    await conn.presenceSubscribe?.(jid).catch(() => {})
    await delay(60)
    await conn.sendReceipt?.(jid, [], 'read').catch(() => {})
    await delay(60)
  } catch {}
}

async function recoverBadMac(conn, jid) {
  try {
    await conn.presenceSubscribe?.(jid).catch(() => {})
    await delay(120)
    await conn.sendReceipt?.(jid, [], 'read').catch(() => {})
...
async function safeSendMessage(conn, jidInput, content, options = {}) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (['NO_SESSIONS', 'NOT_ACCEPTABLE', 'BOOM_5XX', 'TIMEOUT', 'CONN_CLOSED', 'STREAM_ERROR', 'CONFLICT'].includes(cls.type)) {
        await tryRehydrateSession(conn, jid)
      }
...
function createPluginRunner({ conn, concurrency = 128, timeoutMs = 12000 } = {}) {
  ...
  async function run(task, ctx) {
    const id = laneId(ctx)
    if (!lanes.has(id)) lanes.set(id, Promise.resolve())
    const prev = lanes.get(id)
    const p = prev.then(() => queue.run(() => task()))
    lanes.set(id, p.catch(NIL))
    return p
  }
  return { run }
}
  ...
  return function wrap(handlerFn) {
    return async function wrappedHandler(ctx) {
      ...
      const safeCtx = {
        ...
        sender,
        chat,
        normalizeJid: (x) => normalizeJid(x),
        send: (c, o) => safeSendMessage(conn, chat, c, o),
        reply: (c, o) => safeSendMessage(conn, chat, c, { quoted: ctx?.m, ...(o || {}) })
      }
      try {
        ...
        else if (['TIMEOUT', 'CONN_CLOSED', 'STREAM_ERROR', 'CONFLICT'].includes(cls.type)) userMsg = '⚠️ Conexión inestable, reintentando…'
